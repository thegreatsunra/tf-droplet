locals {
  ...
  user_data_vars = {
    ...
    nginx_http_conf_base64  = base64encode(templatefile("./templates/nginx.http.conf.tftpl", local.nginx_conf_vars))
    nginx_https_conf_base64 = base64encode(templatefile("./templates/nginx.https.conf.tftpl", local.nginx_conf_vars))
